...
def generate_implementation(name, info, left_t, right_t):
    ...
    left_expr  = lambda i : f"(({left_t}*)left->data)[{i}]"
    right_expr = lambda i : f"(({right_t}*)right->data)[{i}]"
    ...
    print( "\tu64 stepl = 0, stepr = 0;")
    print( "\tu64 shape;")
    print( "\tif (left->shape == right->shape) {")
    print( "\t\tshape = left->shape;")
    print( "\t\tstepl = 1;")
    print( "\t\tstepr = 1;")
    print( "\t} else if (left->shape == 1) {")
    print( "\t\tshape = right->shape;")
    print( "\t\tstepl = 0;")
    print( "\t\tstepr = 1;")
    print( "\t} else if (right->shape == 1) {")
    print( "\t\tshape = left->shape;")
    print( "\t\tstepl = 1;")
    print( "\t\tstepr = 0;")
    ...
    print( "")
    print( "\tfor (u64 i = 0, j = 0, k = 0; i < shape; ++i, j += stepl, k += stepr) {")
    print(f"\t\t(({result_type}*)result->data)[i] = {op(left_expr('j'), right_expr('k'))};")
    print( "\t}")
    print( "")
